...
private:
  ...
  void rewriteDynamicGEPsForAlloca(AllocaInst &AI, const DataLayout &DL);

  bool shouldExpandDynamicGEP(GetElementPtrInst *GEP, unsigned &DynOpIdx,
                              unsigned &Bound, const DataLayout &DL,
                              unsigned Threshold = 8);
  void expandGEPAndRewriteUsers(GetElementPtrInst *GEP, unsigned DynIdxOp,
                                unsigned Bound, LLVMContext &Ctx);
  bool splitDirLightsStruct(AllocaInst *StructAI,
                            SmallVectorImpl<AllocaInst *> &NewAllocas,
                            const DataLayout &DL);
public:
...
bool AMDGPUPromoteAllocaImpl::run(Function &F, bool PromoteToLDS) {
  ...
  SmallVector<AllocaInst *, 4> ExtraAllocas;
  for (AllocaInst *AI : Allocas)
    splitDirLightsStruct(AI, ExtraAllocas, *DL);
  Allocas.append(ExtraAllocas.begin(), ExtraAllocas.end());
  llvm::erase_if(Allocas, [](AllocaInst *AI) {
    return !AI || AI->getParent() == nullptr;
  });

  sortAllocasToPromote(Allocas);

  // Promote dynmic index alloc
  for (AllocaInst *AI : Allocas)
    rewriteDynamicGEPsForAlloca(*AI, *DL);
...
static void rewriteConstByteGEPs(AllocaInst *StructAI,
                                 ArrayRef<AllocaInst *> FieldAI,
                                 StructType *ST, const DataLayout &DL) {
  SmallVector<Instruction *> Dead;
  const StructLayout *SL = DL.getStructLayout(ST);
  LLVMContext &Ctx = StructAI->getContext();
  Type *I8Ty = Type::getInt8Ty(Ctx);
  Type *I32Ty = Type::getInt32Ty(Ctx);
  for (User *U : llvm::make_early_inc_range(StructAI->users())) {
    auto *GEP = dyn_cast<GetElementPtrInst>(U);
    if (!GEP || !GEP->getType()->isPointerTy())
      continue;
    if (GEP->getNumOperands() != 2)
      continue;
    auto *Off = dyn_cast<ConstantInt>(GEP->getOperand(1));
    if (!Off)
      continue;
    uint64_t ByteOff = Off->getZExtValue();
    unsigned FIdx = ST->getNumElements();
    for (unsigned i = 0, e = ST->getNumElements(); i != e; ++i) {
      uint64_t Beg = SL->getElementOffset(i);
      uint64_t End = Beg + DL.getTypeStoreSize(ST->getElementType(i));
      if (ByteOff >= Beg && ByteOff < End) {
        FIdx = i;
        ByteOff -= Beg;
        break;
      }
    }
    if (FIdx == ST->getNumElements() || !FieldAI[FIdx])
      continue;
    IRBuilder<> B(GEP);
    Value *Base =
        B.CreateBitCast(FieldAI[FIdx], PointerType::get(I8Ty, AMDGPUAS::PRIVATE_ADDRESS));
    Value *NewPtr = B.CreateGEP(I8Ty, Base,
                                ConstantInt::get(I32Ty, ByteOff), GEP->getName());
...
bool AMDGPUPromoteAllocaImpl::splitDirLightsStruct(
    AllocaInst *StructAI, SmallVectorImpl<AllocaInst *> &NewAllocas,
    const DataLayout &DL) {

  auto *ST = dyn_cast<StructType>(StructAI->getAllocatedType());
  if (!ST)
    return false;

  const unsigned N = ST->getNumElements();
  SmallVector<AllocaInst *> FieldAI(N, nullptr);
  bool AnySplit = false;

  auto MakeAI = [&](Type *Ty, Align Al, const Twine &Suf) -> AllocaInst * {
    return new AllocaInst(Ty, AMDGPUAS::PRIVATE_ADDRESS, nullptr, Al,
                          StructAI->getName() + Suf, StructAI);
  };

  for (unsigned Idx = 0; Idx < N; ++Idx) {
    Type *Ty = ST->getElementType(Idx);
    bool Need = isa<FixedVectorType>(Ty);
    if (!Need)
      if (auto *AT = dyn_cast<ArrayType>(Ty)) {
        do {
          Ty = AT->getElementType();
        } while ((AT = dyn_cast<ArrayType>(Ty)));
        Need = isa<FixedVectorType>(Ty);
      }
    if (!Need)
      continue;
    Align Al = DL.getABITypeAlign(ST->getElementType(Idx));
    FieldAI[Idx] = MakeAI(ST->getElementType(Idx), Al,
                          Twine(".field") + Twine(Idx));
    NewAllocas.push_back(FieldAI[Idx]);
    AnySplit = true;
  }
  ...
  SmallVector<Instruction *> DeadGEPs;
  ...
  for (User *U : llvm::make_early_inc_range(StructAI->users())) {
    ...
    auto *F = dyn_cast<ConstantInt>(GEP->getOperand(2));
    if (!Z || !Z->isZero() || !F)
      continue;
    unsigned FI = F->getZExtValue();
    if (FI >= N || !FieldAI[FI])
      continue;
    IRBuilder<> B(GEP);
    SmallVector<Value *> Idx;
    Idx.push_back(ConstantInt::get(Type::getInt32Ty(GEP->getContext()), 0));
    for (unsigned i = 3, e = GEP->getNumOperands(); i < e; ++i)
      Idx.push_back(GEP->getOperand(i));
    Value *NewGEP =
        B.CreateGEP(FieldAI[FI]->getAllocatedType(), FieldAI[FI], Idx,
                    GEP->getName());
    GEP->replaceAllUsesWith(NewGEP);
    DeadGEPs.push_back(GEP);
  }

  rewriteConstByteGEPs(StructAI, FieldAI, ST, DL);

  for (Instruction *I : DeadGEPs)
    I->eraseFromParent();

  bool Erased = false;
  if (StructAI->use_empty()) {
    StructAI->eraseFromParent();
    Erased = true;
    NewAllocas.push_back(nullptr);
  }
  return Erased;
}
...
bool AMDGPUPromoteAllocaImpl::shouldExpandDynamicGEP(GetElementPtrInst *GEP,
                                                     unsigned &DynOpIdx,
                                                     unsigned &Bound,
                                                     const DataLayout &DL,
                                                     unsigned Threshold) {
  unsigned OperandIdx = -1u;
  Bound = 0;

  if (GEP->getPointerAddressSpace() != AMDGPUAS::PRIVATE_ADDRESS)
    return false;

  SmallVector<Value *> ConstIdx;
  for (unsigned i = 1, e = GEP->getNumOperands(); i < e; ++i) {
    ...
    if (!isa<Constant>(Idx)) {
      ...
      Type *IndexedTy =
          GEP->getIndexedType(GEP->getSourceElementType(), ConstIdx);
      if (!IndexedTy)
        return false;

      if (auto *ArrTy = dyn_cast<ArrayType>(IndexedTy)) {
        Bound = ArrTy->getNumElements();
        if (Bound > Threshold)
          return false;
      } else if (auto *VecTy = dyn_cast<FixedVectorType>(IndexedTy)) {
        Bound = VecTy->getNumElements();
      } else {
        return false;
      }
    } else {
...
void AMDGPUPromoteAllocaImpl::expandGEPAndRewriteUsers(GetElementPtrInst *GEP,
                                                       unsigned DynIdxOp,
                                                       unsigned Bound,
                                                       LLVMContext &Ctx) {
  IRBuilder<> B(GEP);
  Value *DynIndex = GEP->getOperand(DynIdxOp);
  SmallVector<GetElementPtrInst *> ConstGEPs;

  for (unsigned i = 0; i < Bound; ++i) {
    auto *Clone = cast<GetElementPtrInst>(GEP->clone());
    Constant *CI = ConstantInt::get(DynIndex->getType(), i);
    Clone->setOperand(DynIdxOp, CI);
    Clone->insertBefore(GEP);
    ConstGEPs.push_back(Clone);
  }

  SmallVector<Instruction *> ToErase;
  for (User *U : llvm::make_early_inc_range(GEP->users())) {
    if (auto *Ld = dyn_cast<LoadInst>(U)) {
      Value *Sel = PoisonValue::get(Ld->getType());
      for (int I = Bound - 1; I >= 0; --I) {
        Value *Cond =
            B.CreateICmpEQ(DynIndex, ConstantInt::get(DynIndex->getType(), I));
        Sel = B.CreateSelect(Cond, B.CreateLoad(Ld->getType(), ConstGEPs[I]),
                             Sel);
      }
      Ld->replaceAllUsesWith(Sel);
      ...
      for (unsigned I = 0; I < Bound; ++I) {
        Value *Cond =
            B.CreateICmpEQ(DynIndex, ConstantInt::get(DynIndex->getType(), I));
        Value *Old = B.CreateLoad(Val->getType(), ConstGEPs[I]);
        Value *NewV = B.CreateSelect(Cond, Val, Old);
        B.CreateStore(NewV, ConstGEPs[I]);
      }
...
void AMDGPUPromoteAllocaImpl::rewriteDynamicGEPsForAlloca(
    AllocaInst &AI, const DataLayout &DL) {
  SmallVector<Instruction *> WorkList;
  for (User *U : AI.users())
    if (auto *GEP = dyn_cast<GetElementPtrInst>(U)) {
      WorkList.push_back(GEP);
      llvm::errs() << "GEP instrunction is: \n";
      GEP->dump();
    }

  for (Instruction *I : WorkList) {
    auto *GEP = cast<GetElementPtrInst>(I);
    unsigned OpIdx, Bound;
    if (shouldExpandDynamicGEP(GEP, OpIdx, Bound, DL))
      expandGEPAndRewriteUsers(GEP, OpIdx, Bound, AI.getContext());
  }
